...
import { PlusIcon, PencilIcon, TrashIcon, SparklesIcon } from '../components/Icons.js';
import type { StoreSummary, Store, DateFilter, AreaStoreFilterState, FilterableData, ModalState, UserProfile, DailyMetric } from '../types.js';
...
const StoresPage: React.FC<StoresPageProps> = ({ 
    storeSummary, allStores, allDateData, dateFilter, setDateFilter, areaStoreFilter, setAreaStoreFilter, setModalState, onEdit, onDelete, onSelectStore, isRecalculating, profile, allMetrics
}) => {
  const { t } = useLocale();
  const canManageStores = profile?.role === 'admin' || profile?.role === 'general_manager';
  ...
  if (canManageStores) {
    columns.push({ key: 'actions', label: t('actions'), render: (item) => (
      <div className="flex space-x-2">
        <button onClick={() => onEdit(item)} className="text-blue-600 hover:text-blue-800 p-1"><PencilIcon /></button>
        <button onClick={() => onDelete(item.id, item.name)} className="text-red-600 hover:text-red-800 p-1"><TrashIcon /></button>
      </div>
    )});
  }
