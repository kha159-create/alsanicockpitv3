...
const LoginPage: React.FC<LoginPageProps> = ({ onSwitchToSignUp }) => {
  const { t } = useLocale();
  const [identifier, setIdentifier] = useState('admin@alsani.com');
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!identifier.includes('@')) {
        ...
        const snapshot = await usersRef.where('employeeId', '==', identifier).limit(1).get();
      ...
      <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-xl shadow-lg">
        ...
        <form className="space-y-6" onSubmit={handleLogin}>
          <div>
            ...
            <input type="text" value={identifier} onChange={(e) => setIdentifier(e.target.value)} required className="input" placeholder={t('email_or_id_placeholder')} />
          </div>
          <div>
