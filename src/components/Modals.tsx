...
import { generateText, generatePrediction } from '../services/geminiService.js';
import type { Store, Employee, ProductSummary, AppMessage, StoreSummary, EmployeeSummary, KPIData, UserProfile, UserRole, DailyMetric, PredictionResult } from '../types.js';
import { useLocale } from '../context/LocaleContext.js';
import { ChartCard, BarChart } from './DashboardComponents.js';
...
export const EmployeeModal: React.FC<EmployeeModalProps> = ({ data, onSave, onClose, isProcessing, stores }) => {
    const { t } = useLocale();
    ...
    return (
        <div className="modal-content">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div><label className="label">{t('employee_name')}</label><input type="text" value={name} onChange={e => setName(e.target.value)} required className="input" /></div>
                <div><label className="label">{t('area_manager')}</label><select value={areaManager} onChange={e => { setAreaManager(e.target.value); setStore(''); }} required className="input"><option value="">{t('select_area')}</option>{areaManagers.map(am => <option key={am} value={am}>{am}</option>)}</select></div>
                <div><label className="label">{t('store')}</label><select value={store} onChange={e => setStore(e.target.value)} required className="input" disabled={!areaManager}><option value="">{t('select_store')}</option>{availableStores.map(s => <option key={s.id} value={s.name}>{s.name}</option>)}</select></div>
...
export const StoreModal: React.FC<StoreModalProps> = ({ data, onSave, onClose, isProcessing }) => {
    const { t } = useLocale();
    const [name, setName] = useState(data?.name || '');
    ...
    return (
        <div className="modal-content">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div><label className="label">{t('store_name')}</label><input type="text" value={name} onChange={e => setName(e.target.value)} required className="input" /></div>
                <div><label className="label">{t('area_manager')}</label><input type="text" value={areaManager} onChange={e => setAreaManager(e.target.value)} required className="input" placeholder="e.g., Central Region"/></div>
                <div className="p-4 border rounded-lg space-y-2">
