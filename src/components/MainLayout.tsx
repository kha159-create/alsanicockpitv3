...
const NavItem: React.FC<NavItemProps> = ({ icon, label, name, activeTab, setActiveTab, setIsSidebarOpen }) => {
  ...
  return (
    <li onClick={handleClick} className={`flex items-center p-3 my-1 rounded-lg cursor-pointer transition-all duration-200 ${isActive ? 'bg-orange-100 text-orange-600 font-semibold' : 'text-zinc-600 hover:bg-gray-100'}`}>
      {icon}
...
const MainLayout: React.FC<MainLayoutProps> = ({ user, profile }) => {
  ...
  // RBAC: Set initial filter state based on user role
  ...
  useEffect(() => {
    ...
    // Conditionally watch 'users' collection for admins/GMs
    ...
    if (profile?.employeeId) {
        tasksUnsubscriber = db.collection('tasks')
            .where('recipientId', '==', profile.employeeId)
...
useEffect(() => {
    ...
    const unsub = db.collection('users')
      ...
      .onSnapshot(snapshot => {
        ...
        // Add new notifications and remove notifications for users who are no longer pending
...
const handleNotificationClick = (notificationId: string) => {
  ...
  const processedData = useDataProcessing({
    ...
    profile, // RBAC: Pass profile to data processing hook
  });

  // FIX: Corrected a typo in a variable name from `kingDovetSales` to `kingDuvetSales`.
  ...
  const runWithRecalculation = useCallback((setter: React.Dispatch<React.SetStateAction<any>>, value: any) => {
    ...
    setter(value);
    setTimeout(() => setIsRecalculating(false), 50); // Small timeout to allow state to propagate
  }, []);
  ...
  // --- CRUD and Action Handlers ---
    const handleSave = async (collectionName: 'stores' | 'employees', data: any) => {
        ...
        try {
            ...
            // FIX: Corrected property name from 'store' to 'currentStore' when saving employee data to match the Employee type definition.
    ...
    const handleDailyMetricSave = async (metricData: any) => {
        setIsProcessing(true);
        try {
            // metricData.date is a string from the form. Convert to Timestamp.
  ...
  const handleLogout = async () => {
    // FIX: Switched to compat API for signOut to resolve module import errors.
    await auth.signOut();
  };
  
    // RBAC: Define visible pages based on role
    ...
    useEffect(() => {
        // If the active tab is no longer visible due to role change, default to the first visible one.
   ...
   const renderContent = () => {
    ...
    const pageProps = {
        ...
        setModalState,
        isRecalculating,
        profile, // RBAC: Pass profile to all pages
    };
      ...
      case 'stores':
       ...
       case 'settings':
          return <SettingsPage
                    ...
                    // FIX: Pass setModalState to SettingsPage to fix missing prop error.
                    setModalState={setModalState}
                 />;
      default:
        return (
  ...
  // FIX: Sidebar transform logic was complex and buggy. Refactored to programmatically set the correct off-screen class based on locale.
  ...
  return (
    <div className="relative md:flex">
      ...
      <aside className={`w-64 bg-white border-r border-gray-200 h-screen p-4 flex flex-col fixed inset-y-0 ltr:left-0 rtl:right-0 transform ${isSidebarOpen ? 'translate-x-0' : sidebarOffscreenClass} md:translate-x-0 transition-transform duration-300 ease-in-out z-30`}>
        <div className="flex items-center gap-3 mb-8 px-2">
            <div className="bg-orange-600 text-white rounded-md flex items-center justify-center w-10 h-10 flex-shrink-0">
                <span className="text-xl font-bold">K.A</span>
            </div>
            <h1 className="text-2xl font-bold text-zinc-800">Cockpit</h1>
        </div>
        ...
        <div className="mt-auto pt-4 text-center text-sm text-gray-500">
            Developed by Khalil Alsani
        </div>
      </aside>
      <main className="flex-1 p-4 sm:p-6 md:ltr:ml-64 md:rtl:mr-64">
        <header className="flex justify-between items-center mb-6">
            <button className="md:hidden p-2 text-zinc-600" onClick={() => setIsSidebarOpen(true)}>
                <MenuIcon />
            </button>
            <div className="flex-1 flex justify-center md:justify-start">
              <h2 className="text-3xl font-bold text-zinc-800 capitalize">{t(activeTab.replace(/([A-Z])/g, ' $1').replace('ai', 'AI'))}</h2>
            </div>
        ...
        {modalState.type &&
            <div className="modal-backdrop">
                {modalState.type === 'employee' && <EmployeeModal data={modalState.data} onSave={(data) => handleSave('employees', data)} onClose={() => setModalState({ type: null })} isProcessing={isProcessing} stores={stores} />}
                {modalState.type === 'store' && <StoreModal data={modalState.data} onSave={(data) => handleSave('stores', data)} onClose={() => setModalState({ type: null })} isProcessing={isProcessing} />}
