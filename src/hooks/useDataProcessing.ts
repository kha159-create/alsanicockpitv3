...
export const useDataProcessing = ({
  ...
  const roleFilteredData = useMemo(() => {
    ...
    if (role === 'employee') {
      ...
      visibleEmployees = activeEmployees.filter(e => visibleStoreNames.has(e.currentStore));
    }

    const visibleEmployeeNames = new Set(visibleEmployees.map(e => e.name));
    
    const filterMetrics = (m: DailyMetric) => visibleStoreNames.has(m.store) && (!m.employee || visibleEmployeeNames.has(m.employee));
    const filterSales = (s: SalesTransaction) => visibleStoreNames.has(s['Outlet Name']) && visibleEmployeeNames.has(s['SalesMan Name']);
  ...
  }, [profile, stores, activeEmployees, dailyMetrics, kingDuvetSales, salesTransactions]);
  ...
  const dateFilteredData = useMemo(() => {
    ...
    // FIX: Updated duvet filtering logic to be more specific, requiring both an alias starting with '4' and a name containing 'COMFORTER'.
  ...
  const areaFilteredData = useMemo(() => {
      ...
      const employeeNamesForArea = new Set(roleFilteredData.employees.filter(e => storeNames.has(getEmployeeStoreForPeriod(e, dateFilter))).map(e => e.name));

      return {
          stores: filteredStores,
          employees: roleFilteredData.employees.filter(e => employeeNamesForArea.has(e.name)),
  ...
  const employeeSummary = useMemo(() => {
      ...
      areaFilteredData.employees.forEach(employee => {
          ...
          // Only include if the employee's store for the period is in the filtered list of stores
          if (!areaFilteredData.stores.some(s => s.name === storeForPeriod)) return;

          const metricsForEmployee = areaFilteredData.metrics.filter(m => m.employee === employee.name);
      ...
      return summary;
  }, [areaFilteredData.employees, areaFilteredData.metrics, areaFilteredData.stores, dateFilter]);
  ...
  const commissionData = useMemo((): CommissionStoreData[] => {
      ...
      // FIX: Explicitly type 'allEmployees' to resolve type inference issues with Object.values/flat.
