...
// Corrected to use Firebase v8 syntax
...
${preview}`;
            ...
            const responseText = await generateText(prompt, 'gemini-2.5-flash');
                        ...
                        case 'employee_sales': {
                            if (format === 'flat') {
                                ...
                                if (dateString && outletName && salesmanName) {
                                    ...
                                    const firestoreTimestamp = firebase.firestore.Timestamp.fromDate(new Date(dateString));
